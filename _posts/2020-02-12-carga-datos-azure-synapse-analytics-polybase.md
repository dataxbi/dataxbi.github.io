...
En Azure he creado una nueva cuenta de almacenamiento, del tipo StorageV2 y en las opciones avanzadas he habilitado el espacio de nombre jer�rquico, para de esta forma crear un Data Lake Gen2. Despu�s he creado un contenedor con el nombre `flights` y dentro de �l he creado la carpeta `2019` y he subido hacia ella el archivo CSV. Es recomendable acceder a las cuentas de almacenamiento de Azure utilizando la herramienta gratuita [Azure Storage Explorer](https://azure.microsoft.com/en-us/features/storage-explorer/).  
...
Tambi�n he creado en Azure un recurso Synapse Analytics (SQL Data Warehouse) y ahora utilizar� PolyBase para cargar el archivo. Para ello hay que empezar creando una tabla externa, que tenga como fuente de datos Azure Data Lake, y con el formato de datos apropiado.
...
Aqu� se guardan las credenciales para acceder al Data Lake, y en este caso utilizar� una clave compartida de la cuenta de almacenamiento (ver imagen) que se copia en `SECRET`. En `IDENTITY` se puede utilizar un valor arbitrario.  
...
Aqu� se crea la fuente de datos externa con el nombre `DataLakeStorage`, utilizando las credenciales almacenadas antes, `DataLakeCredential`. Azure Data Lake Gen2 implementa un sistema de archivo compatible con HADOOP, por lo que en la configuraci�n de la fuente de datos se utiliza el tipo `HADOOP`. En `LOCATION` se utiliza el protocolo `abfss://`, con una `s` extra al final porque al crear la cuenta de almacenamiento indicamos el valor por defecto de s�lo permitir transferencias seguras. `flights` es el nombre del contenedor y `acmedatalake` es el nombre da la cuenta de almacenamiento.
...
El nombre del formato es `CsvFileFormat` y los par�metros indican que es un archivo texto con la coma (,) delimitando los campos y las dobles comillas delimitando las cadenas de caracteres, y que empiece a leer los datos a partir de la segunda fila, porque la primera es de encabezados.
...
Consultamos la tabla externa para estar seguro de que se pueden leer los datos. Tenga en cuenta que estos datos no est�n almacenados en la base de datos, sino en el Data Lake.
...
En Azure Synapse Analytics cada almac�n de datos se separa en 60 distribuciones, que se reparten entre los nodos disponibles.  
Por ejemplo:

- con 100 unidades de almacenamiento (DWU), s�lo hay 1 nodo, que contendr� las 60 distribuciones
- con 1000 DWU, hay 2 nodos, por lo que cada uno tendr� 30 distribuciones

Si los datos est�n divididos en varios archivos, estos se podr�n procesar en paralelo, mejorando el rendimiento de la carga.

Para comprobarlo, separ� el archivo CSV en 4 y repet� la carga de datos con distinta configuraci�n, y med� el tiempo.
